...
def draw(pretrain_embed_dimension,
    ...
    average_name = backbone_names[0] + "_" + str(pretrain_embed_dimension) + "_" + str(target_embed_dimension) \
                        + "_" + "_".join(layers_to_extract_from) + "_" + "average" + "_result.csv"
    ...
    draw_metrics(category, "NMI", NMI_supervised, NMI_unsupervised, NMI_average,
                 "result/" + backbone_names[0] + "_" + str(pretrain_embed_dimension) + "_" + str(target_embed_dimension)
                 + "_" + "_".join(layers_to_extract_from))
    draw_metrics(category, "ARI", ARI_supervised, ARI_unsupervised, ARI_average,
                 "result/" + backbone_names[0] + "_" + str(pretrain_embed_dimension) + "_" + str(target_embed_dimension)
                 + "_" + "_".join(layers_to_extract_from))
    draw_metrics(category, "F1", F1_supervised, F1_unsupervised, F1_average,
                 "result/" + backbone_names[0] + "_" + str(pretrain_embed_dimension) + "_" + str(target_embed_dimension)
                 + "_" + "_".join(layers_to_extract_from))
